...
from explainer.vinfor import vinfor
...
def Args():
    ...
    Parser.add_argument('--dataset', type = str,
                        default = 'ba3', help = 'Dataset to train.',
                        choices = ['ba3','mut','vg5'])
    Parser.add_argument('--cuda', type = int, default = 0,
                        help = 'GPU device.')
    Parser.add_argument('--vae_epoch', type = int, default = 10,
                        help = 'Number of auto-encoder train epoch.')
    Parser.add_argument('--exp_epoch', type = int, default = 0,
                        help = 'Number of explainer train epoch.')
    Parser.add_argument('--epoch', type = int, default = 10,
                        help = 'Number of integral model train epoch.')
    Parser.add_argument('--vae_hidden', type = int, default = 64,
                        help = 'hidden dim of Vae.')
    Parser.add_argument('--vae_out', type = int, default = 16,
                        help = 'out dim of Vae.')
    Parser.add_argument('--alpha_hidden', type = int, default = 64,
                        help = 'hidden dim of AlphaNet.')
    Parser.add_argument('--vae_lr', type = float, default = 1e-5,
                        help = 'Auto-Encoder learning rate.')
    Parser.add_argument('--alpha_lr', type = float, default = 1e-6,
                        help = 'Alpha-Net learning rate.')
    Parser.add_argument('--lr', type = float, default = 1e-5,
                        help = 'Global learning rate.')
    Parser.add_argument('--ratio', type = float, default = 0,
                        help = 'Noise ratio.')
    Parser.add_argument('--tau', type = float, default = 0.3,
                        help = 'Temperature in gumbel-softmax distribution.')
    Parser.add_argument('--gamma', type = float, default = 1,
                        help = 'Hyper param between FidLoss and CTSLoss.')
    ...
    Parser.add_argument('--result', type = str, default = './results/VInfoR/',
...
# Attacked dataset
# adv_train, adv_valid, adv_test = get_dataset('adv' + args.dataset,
                                             root['data'])

# Dimension of edge attribute.
# edge_channel = train[0].edge_attr.size(1)
edge_channel = adv_train[0].edge_attr.size(1)
# print('edge dim:',edge_channel)

# Dimension of node attribute.
# node_channel = torch.flatten(train[0].x,1,-1).size(1)
node_channel = torch.flatten(adv_train[0].x,1,-1).size(1)
...
# In[vinfor]
vinfor = vinfor(device, gnn,
                node_channel, edge_channel,
...
alpha_para = vinfor.AlphaNet.parameters()

opt = torch.optim.Adam([{'params':vae_para, 'lr':args.vae_lr},
                        {'params':alpha_para, 'lr':args.alpha_lr}],
                       args.lr)

'''opt for optimizer, sdl for scheduler'''
# opt = torch.optim.Adam(parameters,args.lr)
sdl = ReduceLROnPlateau(opt, mode = 'min', factor = 0.5,
